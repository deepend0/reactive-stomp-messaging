...
public class SimpleBrokerTest {
    ...
    public void shouldSubscribeAndSend() {
        ...
        Awaitility.await().atMost(AWAIT_AT_MOST).pollInterval(AWAIT_POLL_INTERVAL).until(()->messages.equals(receivedMessages));
        simpleBroker.stop();
    }
    ...
    public void shouldUnsubscribeAndStopSending() throws InterruptedException {
        ...
        Awaitility.await().atLeast(AWAIT_POLL_INTERVAL).and().atMost(AWAIT_AT_MOST).pollInterval(AWAIT_POLL_INTERVAL).until(()->{
    ...
    public void shouldHaveMultipleSubscribers() {
        ...
        Awaitility.await().atMost(AWAIT_AT_MOST).pollInterval(AWAIT_POLL_INTERVAL).until(()->messages.equals(receivedMessages1));
        Awaitility.await().atMost(AWAIT_AT_MOST).pollInterval(AWAIT_POLL_INTERVAL).until(()->messages.equals(receivedMessages2));
        Awaitility.await().atMost(AWAIT_AT_MOST).pollInterval(AWAIT_POLL_INTERVAL).until(()->messages.equals(receivedMessages3));
        simpleBroker.stop();
    }
    ...
    public void shouldHaveMultipleTopics() {
        ...
        Awaitility.await().atMost(AWAIT_AT_MOST).pollInterval(AWAIT_POLL_INTERVAL).until(()->messages1.equals(receivedMessages1));
        Awaitility.await().atMost(AWAIT_AT_MOST).pollInterval(AWAIT_POLL_INTERVAL).until(()->messages2.equals(receivedMessages2));
        Awaitility.await().atMost(AWAIT_AT_MOST).pollInterval(AWAIT_POLL_INTERVAL).until(()->messages3.equals(receivedMessages3));
        simpleBroker.stop();
    }
    ...
    public void shouldUnsubscribeAllTopics() {
        ...
        simpleBroker.subscribe(subscriber, "topic2").subscribe().with(m->receivedMessages2.add((String)m));
        simpleBroker.subscribe(subscriber, "topic3").subscribe().with(m->receivedMessages3.add((String)m));
        simpleBroker.disconnect(subscriber);
        ...
        Awaitility.await().atMost(AWAIT_AT_MOST).pollInterval(AWAIT_POLL_INTERVAL).until(receivedMessages1::isEmpty);
        Awaitility.await().atMost(AWAIT_AT_MOST).pollInterval(AWAIT_POLL_INTERVAL).until(receivedMessages2::isEmpty);
        Awaitility.await().atMost(AWAIT_AT_MOST).pollInterval(AWAIT_POLL_INTERVAL).until(receivedMessages3::isEmpty);
        simpleBroker.stop();
    }
}
